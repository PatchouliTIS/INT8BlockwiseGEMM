...
__global__ void fillExpandedPermutationMapsKernel(
    int const* expanded_dest_row_to_expanded_source_row,
    int* expanded_source_row_to_expanded_dest_row, int* permuted_idx,
    int64_t const rows, int64_t const* num_dest_rows, int64_t k) {
  int64_t const expanded_dest_row = blockIdx.x * blockDim.x + threadIdx.x;
  if (expanded_dest_row >= rows) {
    return;
  }
  ...
  if (!CHECK_SKIPPED || expanded_dest_row < *num_dest_rows) {
    permuted_idx[expanded_dest_row] = expanded_source_row;
  }
}

inline void fillExpandedPermutationMapsLauncher(
    int const* expanded_dest_row_to_expanded_source_row,
    int* expanded_source_row_to_expanded_dest_row, int* permuted_idx,
    int64_t const num_rows, int64_t const* num_valid_tokens_ptr, int const k,
    cudaStream_t stream) {
  int64_t const rows = num_rows * k;
  int64_t const threads = 256;
  int64_t const blocks = (rows + threads - 1) / threads;
  bool const check_skipped = num_valid_tokens_ptr != nullptr;
  using FuncPtr = decltype(&fillExpandedPermutationMapsKernel<true>);
  FuncPtr func_map[2] = {&fillExpandedPermutationMapsKernel<false>,
                         &fillExpandedPermutationMapsKernel<true>};
  auto* const kernel = func_map[check_skipped];
  kernel<<<blocks, threads, 0, stream>>>(expanded_dest_row_to_expanded_source_row,
                                         expanded_source_row_to_expanded_dest_row,
                                         permuted_idx, rows, num_valid_tokens_ptr,
                                         k);
}
