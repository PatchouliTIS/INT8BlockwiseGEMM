...
void moe_permute_metadata(
  ...
  fillExpandedPermutationMapsLauncher(
      get_ptr<int>(sorted_row_idx), get_ptr<int>(inv_permuted_idx),
      get_ptr<int>(permuted_idx), n_token, valid_num_ptr, topk, stream);
}
...
void moe_unpermute(
  ...
  TORCH_CHECK(
      ...
      "moe_unpermute: topk_weights must be float32 (got ",
      topk_weights.scalar_type(),
      "); the kernel reinterprets this buffer as float* unconditionally");
...
void moe_permute_metadata(
    ...
    torch::Tensor& expert_first_token_offset,
    torch::Tensor& inv_permuted_idx, torch::Tensor& permuted_idx) {
  TORCH_CHECK(false, "moe_permute_metadata is not supported on CUDA < 12.0");
}
...
// Op registration is handled in csrc/torch_bindings.cpp