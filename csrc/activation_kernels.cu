...
__global__ void act_and_mul_kernel(
  ...
  // Large DP/EP MoE runs can produce activation tensors with more than
  // 2^31 elements. Cast row index and feature stride to int64 before
  // multiplying so the row base pointer does not wrap in 32-bit arithmetic.
  const int64_t token_idx = blockIdx.x;
  const int64_t d64 = static_cast<int64_t>(d);
  const scalar_t* x_ptr = input + token_idx * 2 * d64;
  const scalar_t* y_ptr = x_ptr + d64;
  scalar_t* out_ptr = out + token_idx * d64;
...
__global__ void activation_kernel(
  ...
  const int64_t token_idx = blockIdx.x;
  const int64_t d64 = static_cast<int64_t>(d);
  const scalar_t* in_ptr = input + token_idx * d64;
  scalar_t* out_ptr = out + token_idx * d64;
